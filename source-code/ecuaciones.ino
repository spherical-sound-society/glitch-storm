...
ISR(TIMER1_COMPA_vect) {
  ...
  switch (programNumber) {
    ...
    case 15:
      ...
      value = ((t & (4 << c)) ? ((t * (t ^ t % b) | (t >> b)) >> 1) : (t >> a) | ((t & (c << b)) ? t << 1 : t));
      aTop = 16;
      aBottom = 0;
      bTop = 22;
      bBottom = 0;
...
void buttonsManager() {
  ...
  if (digitalRead(upButtonPin) == LOW) {
    ...
    if ((millis() - button1Timer > longPress1Time) && (isLongPress1Active == false)) {
      ...
      Serial.println("long press OVER");
    ...
    if (isButton1Active == true) {
      ...
      if (isLongPress1Active == true) {
        ...
        isSerialValues = !isSerialValues;
      } else {

        if (programNumber != totalPrograms) {
          programNumber++;
          //delay(100);
          //blinkNTimes(programNumber);
          // out();
        }
  ...
  if (digitalRead(downButtonPin) == LOW) {
    ...
    if ((millis() - button2Timer > longPress2Time) && (isLongPress2Active == false)) {
      ...
      Serial.println("long press OVER button 2");
    ...
    if (isButton2Active == true) {
      ...
      if (isLongPress2Active == true) {
        ...
        Serial.println("long press release button 2");
